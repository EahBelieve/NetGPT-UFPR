"""
run_distillation_slim.py — Knowledge Distillation for NetGPT-Slim
==================================================================
Distills a fine-tuned 12-layer GPT-2 teacher into NetGPT-Slim, a
pruning-informed compact architecture:
  - 6 layers (↓ from 12)
  - 8 attention heads (↓ from 12), per_head_size = 96
  - feedforward_size = 1024 (↓ from 3072)
  - hidden_size = 768 (unchanged)
  - ~25M parameters (3.4× compression from ~84.9M)

The architecture is derived from global pruning analysis which revealed:
  - FFN down-projection layers are ~85% redundant → shrink d_ff
  - Attention layers are moderately/uniformly sparse → reduce heads
  - Embedding dimension carries essential information → keep hidden_size

Initialization strategy (shape-aware):
  - Embeddings: copied as-is (same hidden_size=768)
  - Attention Q/K/V + output: copied as-is (768→768, head reshaping
    changes only at forward time — 12 heads of 64 vs 8 heads of 96)
  - FFN: TRUNCATED — take first d_ff' neurons from teacher's d_ff
    (linear_1: [768,3072]→[768,1024], linear_2: [3072,768]→[1024,768])
  - Output classifier: copied as-is (same hidden_size)
  - Layer selection: DistilBERT-style (layers 1,3,5,7,9,11 → 0..5)
...
Usage:
  python finetune/run_distillation_slim.py \\
    --teacher_model_path  models/finetuned_model.bin \\
    --output_model_path   models/slim_model.bin \\
    --vocab_path          models/encryptd_vocab.txt \\
    --config_path         models/gpt2/config.json \\
    --student_config_path models/gpt2/slim_config.json \\
...
#  Student initialisation — shape-aware for NetGPT-Slim

def init_student_from_teacher(teacher_state, student_model,
                               teacher_layers=12, student_layers=6):
    """
    Shape-aware initialisation for NetGPT-Slim.

    Handles the case where student has different feedforward_size and/or
    heads_num compared to teacher. Strategy:
      - Same-shape tensors: copy as-is
      - Smaller student tensor: TRUNCATE teacher tensor (take first N rows/cols)
      - Larger student tensor: should not happen for Slim, but handled gracefully

    Layer selection: DistilBERT-style — take one layer out of two.
    ...
    stats = {"copied": 0, "truncated": 0, "skipped_layer": 0, "random_init": 0}

    for key, student_val in student_state.items():
        # ── Step 1: Remap layer indices ──
        teacher_key = key
        ...
        for s_idx, t_idx in layer_map.items():
            ...
            if s_pattern in key:
                ...
                teacher_key = key.replace(s_pattern, t_pattern)
                is_layer_param = True
                break

        # Check if this is a layer param not in our selected set
        ...
        # ── Step 2: Find matching teacher weight ──
        if teacher_key not in teacher_state:
            # No teacher weight for this key → keep student's random init
            init_state[key] = student_val
            stats["random_init"] += 1
            continue

        teacher_val = teacher_state[teacher_key]

        # ── Step 3: Shape-aware copy ──
        if teacher_val.shape == student_val.shape:
            # Same shape → copy directly
            init_state[key] = teacher_val.clone()
            stats["copied"] += 1
        elif len(teacher_val.shape) == 2:
            # Matrix: truncate along mismatched dimensions
            s0 = min(teacher_val.shape[0], student_val.shape[0])
            s1 = min(teacher_val.shape[1], student_val.shape[1])
            truncated = teacher_val[:s0, :s1].clone()
            # If student is larger in some dim (shouldn't happen for Slim),
            # pad remaining with student's random init
            result = student_val.clone()
            result[:s0, :s1] = truncated
            init_state[key] = result
            stats["truncated"] += 1
            print(f"  [TRUNCATE] {key}: {list(teacher_val.shape)} → {list(student_val.shape)}")
        elif len(teacher_val.shape) == 1:
            # Bias vector: truncate
            s0 = min(teacher_val.shape[0], student_val.shape[0])
            result = student_val.clone()
            result[:s0] = teacher_val[:s0].clone()
            init_state[key] = result
            stats["truncated"] += 1
            print(f"  [TRUNCATE] {key}: [{teacher_val.shape[0]}] → [{student_val.shape[0]}]")
        else:
            # Higher-dim tensor (shouldn't happen) → keep random
            init_state[key] = student_val
            stats["random_init"] += 1

    print(f"\n  Init summary: {stats['copied']} copied, "
          f"{stats['truncated']} truncated, "
          f"{stats['random_init']} random-init, "
          f"{stats['skipped_layer']} skipped-layers")
...
def main():
    ...
    parser.add_argument("--student_config_path", type=str,
                        default="models/gpt2/slim_config.json",
                        help="Student config (NetGPT-Slim: 6L, 8H, d_ff=1024).")
    ...
    # Shape-aware init for Slim (handles FFN truncation)
    teacher_state = teacher.state_dict()
    print("\n[Init] Transferring teacher → student (shape-aware)...")
    ...
    student.load_state_dict(student_init, strict=True)
    ...
    args.logger.info("=" * 60)
    args.logger.info("  NetGPT-Slim Distillation")
    args.logger.info("=" * 60)
    args.logger.info("Teacher: {:,} params ({} layers, {} heads, d_ff={})".format(
        t_params, teacher_layers, teacher_args.heads_num, teacher_args.feedforward_size))
    args.logger.info("Student: {:,} params ({} layers, {} heads, d_ff={})".format(
        s_params, student_layers, student_args.heads_num, student_args.feedforward_size))
